...
async function updateUserPasswords() {
  ...
  try {
    ...
    const countResult = await pool.request().query("SELECT COUNT(*) as total FROM usuarios WHERE idPerfil = 1");
    ...
    // Atualiza as senhas
    const updateResult = await pool.request().input("newPassword", sql.VarChar, newPassword).query(`
                UPDATE usuarios 
                SET senha = @newPassword, updatedAt = GETDATE()
                WHERE idPerfil = 1
            `);
